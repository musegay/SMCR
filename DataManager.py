    ...
    def __init__(self, stopword_freq_lb, path, no_pretrain_word2vec, dim, context_len):
        ...
        for name in ["train", "valid", "test"]:
            ...
            for line in open(file_path):
                ...
                self.text[name].append(
                    [utterances[-context_len:-1], utterances[-1], entities[cnt], sys_ans_utt_ori[cnt]])
                cnt += 1
        ...
        for name in ["train", "valid", "test"]:
            ...
            for number, item in enumerate(self.text[name]):
                len_u = len(item[0])
                indices = [[], [[] for _ in range(len_u)], [], [], [],
                           []]  # src_len, src, trg, trg_entities, trg_entities_mask
