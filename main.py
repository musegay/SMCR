...
@author: truthless
"""
from MultihopSparseTopicRNN import TopicRNN_SGAT
...
def train():
    ...
    model = TopicRNN_SGAT(args.model, args.rnn, manager.vector, len(manager.word2index), len(manager.index2nonstop), args.embed, args.rnn_dim, args.infer_dim, args.topic, manager.n_entity, nodes_rep, adj, gcn_dropout=0.5, teacher_forcing=args.teacher)
...
def just_test():
    ...
    all_input_sent, all_trg_seqs_ori, all_outputs_ori, all_ent_grd_truth, bleu_score, bleu_score_ori, bleu_score_corpus, bleu_score_ori_corpus, perplexity_score, accuracy = \
    test_model(model, test, 0, manager, log_output, manager.id_nodename_map, "test")

    return bleu_score, bleu_score_ori, bleu_score_corpus, bleu_score_ori_corpus, perplexity_score, accuracy
